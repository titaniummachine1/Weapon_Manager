...
--local mAntiPred        = menu:AddComponent(MenuLib.Checkbox("Anti Prediction",             false))                          -- Leg Jitter
...
--[[
    if mAntiPred:GetValue() == true then                                -- If Leg Jitter is enabled,             -- Check if we are pressing WASD
                                   -- Check if we not currently moving 
            if pCmd.command_number % 2 == 0 then                         -- Check if the command number is even. (Potentially inconsistent, but it works).
                pCmd:SetSideMove(90)                                      -- Cycle between moving left and right possibly faking predicted movement to make lbox miss
            else
                pCmd:SetSideMove(-90)
            end
    ...
    if pLocal:InCond(4) or pLocal:InCond(2) 
        ...
        if (mRetryStunned:GetValue() == true) then                                   -- If Retry when stunned is enabled
            ...
            elseif (pLocal:InCond(7)) and (distance <= 200)                          -- If we are laughing (7 is TF_COND_TAUNTING), and we're within 200hu of the player we are iterating through
                ...
                client.command("kill", true)                                        -- suicide
            end
        end
